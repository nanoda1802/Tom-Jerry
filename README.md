...
- **점수**
  ...
  - [ ] gameEnd() 함수 점수 검증 로직 최적화
    - 이론상 획득 가능한 최대 점수를 초과하진 않는지
    - 서버에서 계산한 총점과 일치하는지 (점수 획득 로직이 바뀌어서 조정해야함)
    - 클리어일 시 플레이 시간이 총 시간과 비교해서 적절한지
    - 검증 실패 시 하이스코어 갱신 X
- **스테이지**
...
- **아이템**
  ...
  - [ ] 아이템 생성 관련 서버 검증 프로세스
    - 생성 주기가 맞는지
    - 생성된 아이템이 스테이지에 적합한 아이템인지
    - 검증 실패 시 생성 X
  - [ ] 코인 획득 관련 서버 검증 프로세스
    - 획득 점수가 아이템 별 점수와 일치하는지
    - 획득 주기가 생성 주기와 비교해서 적절한지
    - 검증 실패 시 점수 획득 X
  - [ ] 별 획득 관련 서버 검증 프로세스
    - 획득 점수가 지정된 점수와 일치하는지
    - 획득 주기가 생성 주기와 비교해서 적절한지
    - 효과 유지 프레임이 데이터 테이블의 정보와 비교해서 적절한지
    - 검증 실패 시 점수 획득 X 효과 즉시 종료
- **채팅**
